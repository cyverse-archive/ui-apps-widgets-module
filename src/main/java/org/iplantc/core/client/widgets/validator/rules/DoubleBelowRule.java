...
/**
 * Rule for enforcing that a string is a valid double-precision number with a value below a user defined
 * threshold.
 * 
 * @author amuir
 * 
 */
public class DoubleBelowRule extends DoubleRule {
    /**
     * Instantiate from a parameter list.
     * 
     * @param params list of parameters to configure rule.
     */
    public DoubleBelowRule(List<String> params) {
        super(params);
    }
    ...
    public String validate(ComponentValueTable tbl, String name, String value) {
        String err = I18N.RULES.notValidDoubleBelowMsg(name, doubleValue);

        if (ValidatorHelper.isDouble(value)) {
            err = I18N.RULES.notBelowValueMsg(name, doubleValue);
